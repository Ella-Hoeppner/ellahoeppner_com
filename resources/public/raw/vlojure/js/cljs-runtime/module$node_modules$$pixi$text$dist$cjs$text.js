...
measured.fontProperties;this.canvas.width=Math.ceil((Math.max(1,measured.width)+2*style.padding)*this._resolution);this.canvas.height=Math.ceil((Math.max(1,height)+2*style.padding)*this._resolution);respectDirty.scale(this._resolution,this._resolution);respectDirty.clearRect(0,0,this.canvas.width,this.canvas.height);respectDirty.font=this._font;respectDirty.lineWidth=style.strokeThickness;respectDirty.textBaseline=style.textBaseline;respectDirty.lineJoin=style.lineJoin;respectDirty.miterLimit=style.miterLimit;
for(var linePositionX,linePositionY,passesCount=style.dropShadow?2:1,i=0;i<passesCount;++i){var dsOffsetText=(linePositionY=style.dropShadow&&0===i)?Math.ceil(Math.max(1,height)+2*style.padding):0;linePositionX=dsOffsetText*this._resolution;linePositionY?(respectDirty.fillStyle="black",respectDirty.strokeStyle="black",linePositionY=style.dropShadowColor,linePositionY=utils.hex2rgb("number"===typeof linePositionY?linePositionY:utils.string2hex(linePositionY)),respectDirty.shadowColor="rgba("+255*linePositionY[0]+
","+255*linePositionY[1]+","+255*linePositionY[2]+","+style.dropShadowAlpha+")",respectDirty.shadowBlur=style.dropShadowBlur,respectDirty.shadowOffsetX=Math.cos(style.dropShadowAngle)*style.dropShadowDistance,respectDirty.shadowOffsetY=Math.sin(style.dropShadowAngle)*style.dropShadowDistance+linePositionX):(respectDirty.fillStyle=this._generateFillStyle(style,lines,measured),respectDirty.strokeStyle=style.stroke,respectDirty.shadowColor="black",respectDirty.shadowBlur=0,respectDirty.shadowOffsetX=
0,respectDirty.shadowOffsetY=0);var linePositionYShift=(lineHeight-fontProperties.fontSize)/2;if(!Text.nextLineHeightBehavior||0>lineHeight-fontProperties.fontSize)linePositionYShift=0;for(var i_1=0;i_1<lines.length;i_1++)linePositionX=style.strokeThickness/2,linePositionY=style.strokeThickness/2+i_1*lineHeight+fontProperties.ascent+linePositionYShift,"right"===style.align?linePositionX+=maxLineWidth-lineWidths[i_1]:"center"===style.align&&(linePositionX+=(maxLineWidth-lineWidths[i_1])/2),style.stroke&&
style.strokeThickness&&this.drawLetterSpacing(lines[i_1],linePositionX+style.padding,linePositionY+style.padding-dsOffsetText,!0),style.fill&&this.drawLetterSpacing(lines[i_1],linePositionX+style.padding,linePositionY+style.padding-dsOffsetText)}this.updateTexture()}};Text.prototype.drawLetterSpacing=function(text,x,y,isStroke){void 0===isStroke&&(isStroke=!1);var letterSpacing=this._style.letterSpacing;if(0===letterSpacing)isStroke?this.context.strokeText(text,x,y):this.context.fillText(text,x,y);
else for(var stringArray=Array.from?Array.from(text):text.split(""),previousWidth=this.context.measureText(text).width,currentWidth,i=0;i<stringArray.length;++i)currentWidth=stringArray[i],isStroke?this.context.strokeText(currentWidth,x,y):this.context.fillText(currentWidth,x,y),currentWidth=this.context.measureText(text.substring(i+1)).width,x+=previousWidth-currentWidth+letterSpacing,previousWidth=currentWidth};Text.prototype.updateTexture=function(){var canvas=this.canvas;if(this._style.trim){var trimmed=
utils.trimCanvas(canvas);trimmed.data&&(canvas.width=trimmed.width,canvas.height=trimmed.height,this.context.putImageData(trimmed.data,0,0))}trimmed=this._texture;var style=this._style;style=style.trim?0:style.padding;var baseTexture=trimmed.baseTexture;trimmed.trim.width=trimmed._frame.width=Math.ceil(canvas.width/this._resolution);trimmed.trim.height=trimmed._frame.height=Math.ceil(canvas.height/this._resolution);trimmed.trim.x=-style;trimmed.trim.y=-style;trimmed.orig.width=trimmed._frame.width-
2*style;trimmed.orig.height=trimmed._frame.height-2*style;this._onTextureUpdate();baseTexture.setRealSize(canvas.width,canvas.height,this._resolution);this._recursivePostUpdateTransform();this.dirty=!1};Text.prototype._render=function(renderer){this._autoResolution&&this._resolution!==renderer.resolution&&(this._resolution=renderer.resolution,this.dirty=!0);this.updateText(!0);_super.prototype._render.call(this,renderer)};Text.prototype.getLocalBounds=function(rect){this.updateText(!0);return _super.prototype.getLocalBounds.call(this,
rect)};Text.prototype._calculateBounds=function(){this.updateText(!0);this.calculateVertices();this._bounds.addQuad(this.vertexData)};Text.prototype._generateFillStyle=function(style,lines,metrics){var fillStyle=style.fill;if(!Array.isArray(fillStyle))return fillStyle;if(1===fillStyle.length)return fillStyle[0];var dropShadowCorrection=style.dropShadow?style.dropShadowDistance:0,padding=style.padding||0,width=Math.ceil(this.canvas.width/this._resolution)-dropShadowCorrection-2*padding,height=Math.ceil(this.canvas.height/
this._resolution)-dropShadowCorrection-2*padding;dropShadowCorrection=fillStyle.slice();var fillGradientStops=style.fillGradientStops.slice();if(!fillGradientStops.length)for(var lengthPlus1=dropShadowCorrection.length+1,i=1;i<lengthPlus1;++i)fillGradientStops.push(i/lengthPlus1);dropShadowCorrection.unshift(fillStyle[0]);fillGradientStops.unshift(0);dropShadowCorrection.push(fillStyle[fillStyle.length-1]);fillGradientStops.push(1);if(style.fillGradientType===exports.TEXT_GRADIENT.LINEAR_VERTICAL)for(fillStyle=
this.context.createLinearGradient(width/2,padding,width/2,height+padding),style=(metrics.fontProperties.fontSize+style.strokeThickness)/height,i=0;i<lines.length;i++)for(padding=metrics.lineHeight*i,width=0;width<dropShadowCorrection.length;width++)lengthPlus1=Math.min(1,Math.max(0,padding/height+("number"===typeof fillGradientStops[width]?fillGradientStops[width]:width/dropShadowCorrection.length)*style)),lengthPlus1=Number(lengthPlus1.toFixed(5)),fillStyle.addColorStop(lengthPlus1,dropShadowCorrection[width]);
else for(fillStyle=this.context.createLinearGradient(padding,height/2,width+padding,height/2),lines=dropShadowCorrection.length+1,metrics=1,i=0;i<dropShadowCorrection.length;i++)fillStyle.addColorStop("number"===typeof fillGradientStops[i]?fillGradientStops[i]:metrics/lines,dropShadowCorrection[i]),metrics++;return fillStyle};Text.prototype.destroy=function(options){"boolean"===typeof options&&(options={children:options});options=Object.assign({},defaultDestroyOptions,options);_super.prototype.destroy.call(this,
options);this._ownCanvas&&(this.canvas.height=this.canvas.width=0);this._style=this.canvas=this.context=null};Object.defineProperty(Text.prototype,"width",{get:function(){this.updateText(!0);return Math.abs(this.scale.x)*this._texture.orig.width},set:function(value){this.updateText(!0);var s=utils.sign(this.scale.x)||1;this.scale.x=s*value/this._texture.orig.width;this._width=value},enumerable:!1,configurable:!0});Object.defineProperty(Text.prototype,"height",{get:function(){this.updateText(!0);return Math.abs(this.scale.y)*
this._texture.orig.height},set:function(value){this.updateText(!0);var s=utils.sign(this.scale.y)||1;this.scale.y=s*value/this._texture.orig.height;this._height=value},enumerable:!1,configurable:!0});Object.defineProperty(Text.prototype,"style",{get:function(){return this._style},set:function(style){style=style||{};this._style=style instanceof TextStyle?style:new TextStyle(style);this.localStyleID=-1;this.dirty=!0},enumerable:!1,configurable:!0});Object.defineProperty(Text.prototype,"text",{get:function(){return this._text},
set:function(text){text=String(null===text||void 0===text?"":text);this._text!==text&&(this._text=text,this.dirty=!0)},enumerable:!1,configurable:!0});Object.defineProperty(Text.prototype,"resolution",{get:function(){return this._resolution},set:function(value){this._autoResolution=!1;this._resolution!==value&&(this._resolution=value,this.dirty=!0)},enumerable:!1,configurable:!0});Text.nextLineHeightBehavior=!1;return Text}(global.Sprite);exports.Text=require;exports.TextMetrics=TextMetrics$jscomp$0;
